...
# default_json_rules + additional_json_rules: to generate the base default rules and merge additional partial
...
variable "default_json_rules" {
  description = "Flag to setup the base default JSON rules for the Property generated by the module"
  type        = bool
  default     = false
  validation {
    condition     = (length(try(var.custom_json_rules, "")) > 0) || var.default_json_rules
    error_message = "Either 'custom_json_rules' or 'default_json_rules' must be provided."
  }
}
# Additional partial rules to be merged into the Property rules
variable "additional_json_rules" {
  description = "list of JSON strings representing a partial sets of rules for the Property to be merged into the base rules"
  type        = list(string)
  default     = null
}
