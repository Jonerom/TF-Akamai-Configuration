...
variable "host_configuration" {
  ...
  type = map(object({
    ...
    records = list(object({                                    # List of DNS records to be created in the zone
      ...
      cert_provisioning_type = optional(string, "CPS_MANAGED") # Certificate provisioning type for the hostname, possible values: CPS_MANAGED, DEFAULT or CCM
...
variable "default_json_rule_values" {
  ...
  type = object({
    ...
    origin_type = optional(string, "CUSTOMER") # Only origin type supported by TF API is: CUSTOMER. UI allows also the usage of NET_STORAGE or AKAMAI_OBJECT_STORAGE
    ...
    custom_valid_cn_values        = optional(list(string), null)            # Custom common name values for origin certificate verification when verification_mode is CUSTOM
    ...
    caching_behavior      = optional(string, "NO_STORE") # Caching behavior option in the mandatory rules, possible values: "NO_STORE", "BYPASS_CACHE", "MAX_AGE", "EXPIRES", "CACHE_CONTROL" or "CACHE_CONTROL_AND_EXPIRES"
...
variable "additional_json_rules" {
  ...
  default     = []
  validation {
    condition     = ((var.additional_json_rules != null && (length(try(var.additional_json_rules, "")) > 0)) || var.basic_json_rules)
