...
variable "security_config" {
  ...
  type = object({
    evasive_path_match_enable = optional(bool, true)  # Enable Evasive URL Request Matching
    prefetch_enable_app_layer = optional(bool, true)  # Enable Prefetch Requests for Application Layer
    prefetch_all_extensions   = optional(bool, false) # Prefetch Requests for All Extensions if prefetch_enable_app_layer is true
    prefetch_extensions = optional(                   # List of Extensions for Prefetch Requests if prefetch_all_extensions is false and prefetch_enable_app_layer is true it must be empty []
      list(string),
      ["cgi", "jsp", "aspx", "EMPTY_STRING", "php", "py", "asp"]
    )
    prefetch_enable_rate_controls = optional(bool, false)  # Enable Rate Controls for Prefetch Requests
    request_body_inspection_limit = optional(string, "32") # Request size inspection limit in KB, possible values: default, 8, 16, 32
    pii_learning_enable           = optional(bool, false)  # Enable API PII learning
    http_logging = optional(object({                       # Establish HTTP header data logging configuration
      enabled       = optional(string, "true")             # Enable HTTP header logging
      cookies       = optional(string, "all")              # Cookie headers to log, possible values: all, none, exclude, only
      custom_type   = optional(string, "all")              # Custom headers to log, possible values: all, none, exclude, only
      standard_type = optional(string, "all")              # Standard headers to log, possible values: all, none, exclude, only
    }))
    attack_payload_logging = object({                    # Establish Attack payload logging configuration
      enabled       = optional(string, "true")           # Enable Attack payload logging
      request_body  = optional(string, "ATTACK_PAYLOAD") # Log request body, possible values: NONE or ATTACK_PAYLOAD
      response_body = optional(string, "ATTACK_PAYLOAD") # Log response body, possible values: NONE or ATTACK_PAYLOAD
    })
    siem_settings_enable         = optional(bool, true)   # Enable SIEM integration
    siem_enable_for_all_policies = optional(bool, true)   # Enable SIEM integration for all security policies
    ...
    siem_id                      = optional(number, 1)    # SIEM integration ID to use
    siem_include_ja4_fingerprint = optional(bool, false)  # Include JA4 fingerprint in SIEM logs
    ...
    pragma_header = object({                              # Establish Strip Pragma debug headers from responses configuration
      action                 = optional(string, "REMOVE") # Action to apply to Pragma debug headers, possible values: REMOVE
